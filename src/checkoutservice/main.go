...
func main() {
	...
	si := grpctrace.StreamServerInterceptor(grpctrace.WithServiceName("checkoutservice"))
	ui := grpctrace.UnaryServerInterceptor(grpctrace.WithServiceName("checkoutservice"))
...
func (cs *checkoutService) PlaceOrder(ctx context.Context, req *pb.PlaceOrderRequest) (*pb.PlaceOrderResponse, error) {
	...
	for _, it := range prep.orderItems {
		multPrice := money.MultiplySlow(ctx, *it.Cost, uint32(it.GetItem().GetQuantity()))
